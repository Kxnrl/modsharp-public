...
public class HookExample : IModSharpModule
{
    ...
    private IVirtualHook? _vmtHook;
    private IDetourHook?  _detourHook;
    ...
    // Our hook functions (like `hk_CCSPlayerPawn_CollisionRulesChanged`) must be `static`.
    ...
    //
    // However, a static method cannot directly access instance members like `_shared` or `_vmtHook`.
    ...
    private unsafe bool InstallVmtHook()
    {
        ...
        _vmtHook = _shared.GetHookManager().CreateVirtualHook();
        ...
        // for the method (`hk_CCSPlayerPawn_CollisionRulesChanged`) which can be called from native code with minimal overhead,
        // just like any other native function.
        _vmtHook.Prepare("server",
                         "CCSPlayerPawn",
                         vFuncIndex,
                         (nint) (delegate* unmanaged<nint, void>) (&hk_CCSPlayerPawn_CollisionRulesChanged));
        ...
        _vmtHook.Prepare(vtableAddress, vFuncIndex, (nint) (delegate* unmanaged<nint, void>) (&hk_CCSPlayerPawn_CollisionRulesChanged));
        */
        ...
        CCSPlayerPawn_CollisionRulesChangedOriginal = (delegate* unmanaged<nint, void>) _vmtHook.Trampoline;
    ...
    private unsafe bool InstallDetour()
    {
        ...
        _detourHook.Prepare("CreateEntityByName", (nint) (delegate* unmanaged<byte*, int, nint>) (&hk_CreateEntityByName));
        ...
        _detourHook.Prepare(createEntityByNameAddress, (nint) (delegate* unmanaged<byte*, int, nint>) (&hk_CreateEntityByName));
        */
    ...
    private static unsafe void hk_CCSPlayerPawn_CollisionRulesChanged(nint @this)
    {
        ...
        Console.WriteLine("[hk_CCSPlayerPawn_CollisionRulesChanged] entity.Classname: {0}", entity.Classname);
    ...
    [UnmanagedCallersOnly] // YOU MUST ADD THIS TO YOUR HOOKED FUNCTION
    private static unsafe nint hk_CreateEntityByName(byte* classNamePtr, int forcedIndex)
    {
        ...
        // NOTE: This is for demonstration only and can cause conflicts.
        if (forcedIndex == -1)
        {
        ...
        {
            Console.WriteLine("[hk_CreateEntityByName] Failed to create entity \"{0}\" with forced index \"{1}\"",
                              className,
                              forcedIndex);
        ...
        Console.WriteLine("[hk_CreateEntityByName] Successfully created entity \"{0}\" with forced index \"{1}\". Address: 0x{2:X}",
                          className,
                          forcedIndex,
                          entityPtr);

        // Or we can do things to certain entities
        if (_this._shared.GetEntityManager().MakeEntityFromPointer<IBaseEntity>(entityPtr) is { IsValidEntity: true } entity
            && entity.Classname is "weapon_glock")
        {
